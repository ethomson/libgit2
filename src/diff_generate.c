...
#define DIFF_FLAG_SET(DIFF,FLAG,VAL) (DIFF)->base.opts.flags = \
	(VAL) ? ((DIFF)->base.opts.flags | (FLAG)) : ((DIFF)->base.opts.flags & ~(VAL))
...
static git_diff_generated *diff_list_alloc(
	git_repository *repo,
	git_iterator *old_iter,
	git_iterator *new_iter)
{
	git_diff_options dflt = GIT_DIFF_OPTIONS_INIT;
	git_diff_generated *diff = git__calloc(1, sizeof(git_diff_generated));
	if (!diff)
		return NULL;
	...
	diff->base.find_similar_fn = git_diff_generated_find_similar;
	diff->base.free_fn = diff_generated_free;
	diff->base.repo = repo;
	diff->base.old_src = old_iter->type;
	diff->base.new_src = new_iter->type;
	memcpy(&diff->base.opts, &dflt, sizeof(diff->base.opts));
...
int git_diff_generated__oid_for_file(
	git_oid *out,
	git_diff_generated *diff,
	const char *path,
	uint16_t mode,
	...
	const git_oid *update_match)
{
	...
	if (git_buf_joinpath(
		&full_path, git_repository_workdir(diff->base.repo), entry.path) < 0)
		return -1;
	...
	if (S_ISGITLINK(mode)) {
		...
		&fl, diff->base.repo, NULL, entry.path,
	...
	const git_diff_options *opts)
{
	...
	if (!error) {
		git_diff_generated *dg = (git_diff_generated *)*diff;

		if (DIFF_FLAG_IS_SET(dg, GIT_DIFF_UPDATE_INDEX) && dg->index_updated)
