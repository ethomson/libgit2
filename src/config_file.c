...
static int parse_on_variable(struct reader **reader, const char *current_section, char *var_name, char *var_value, void *data)
{
	...
	if (!git__strcmp(var->entry->name, "include.path")) {
		...
		*reader = git_array_get(parse_data->cfg_file->readers, parse_data->reader_idx);
		...
		if (result == 0) {
			result = config_parse(parse_data->values, parse_data->cfg_file, r, parse_data->level, parse_data->depth+1);
...
static int config_parse(git_strmap *values, diskfile_backend *cfg_file, struct reader *reader, git_config_level_t level, int depth)
{
	...
	return config_iterate(reader, NULL, parse_on_variable, NULL, &parse_data);
}
...
static int config_write(diskfile_backend *cfg, const char *key, const regex_t *preg, const char* value)
{
	...
	if ((result = config_iterate(reader, write_on_section, write_on_variable, write_on_eof, &write_data)) < 0) {
