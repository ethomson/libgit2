...
static int diff_print_info_init_frompatch(
	...
	void *payload)
{
	git_repository *repo = patch && patch->repo ? patch->repo : NULL;
	...
	const git_diff_delta *delta, float progress, void *data)
{
	...
	id_abbrev = min(delta->old_file.id_abbrev, delta->new_file.id_abbrev);
	...
	git_buf *out, const git_diff_delta *delta, int oid_strlen)
{
	...
	if (oid_strlen > id_abbrev + 1) {
		giterr_set(GITERR_PATCH,
			"The patch input contains %d id characters (cannot print %d)",
			id_abbrev, oid_strlen);
		return -1;
	}
