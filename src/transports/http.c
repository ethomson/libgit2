	...
	git_http_response *response)
{
	...
	if (response->server_credtypes == 0) {
		git_error_set(GIT_ERROR_NET, "server requires authentication that we do not support");
		return -1;
	}

	/* TODO: don't set this on transport->server...  transport->creds? */
	...
	size_t *out_len)
{
	...
	*out_len = 0;
	...
	while (stream->state == HTTP_STATE_SENDING_REQUEST &&
		...
		if ((error = git_http_client_send_request(
	...
	size_t len)
{
	...
	while (stream->state == HTTP_STATE_NONE &&
		...
		if ((error = lookup_proxy(&use_proxy, transport)) < 0)
			goto done;

		request.method = GIT_HTTP_METHOD_POST;
		request.url = &url;
		request.chunked = stream->service->chunked;
		request.content_length = stream->service->chunked ? len : 0;
