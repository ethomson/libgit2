...
static int read_header_loose(git_rawobj *out, git_buf *loc)
{
	...
	unsigned char obj[64*2], inflated[HEADER_LEN];
...
printf("start\n");
	...
	goto done;
printf("init\n");
		if ((error = git_zstream_set_input(&zs, obj, obj_len)) < 0)
		goto done;
		printf("input : obj_len=%d\n", obj_len);

		if ((error = git_zstream_get_output_chunk(inflated, &inflated_len, &zs)) < 0)
		goto done;
		printf("output %zu\n", inflated_len);

		if ((error = parse_header(&hdr, &header_len, inflated, inflated_len)) < 0)
		goto done;
		printf("parsed\n");
	...
	loose_readstream *stream)
{
	 ...
	 * binary encoding of the object type and size.
	 */
	if ((error = parse_header_packlike(hdr, &head_len,
