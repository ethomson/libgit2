	...
	size_t size_hint)
{
	...
	if ((error = git_vector_init(&new_entries, size_hint, index->entries._cmp)) < 0 ||
		(error = git_vector_init(&remove_entries, index->entries.length, NULL)) < 0 ||
		(error = git_idxmap_alloc(&new_entries_map)) < 0)
		goto done;
	
	if (index->ignore_case && size_hint)
		kh_resize(idxicase, (khash_t(idxicase) *) new_entries_map, size_hint);
	else if (size_hint)
		kh_resize(idx, new_entries_map, size_hint);

	opts.flags = GIT_ITERATOR_DONT_IGNORE_CASE;

	if ((error = index_sort_if_needed(index, true)) < 0 ||
		(error = git_iterator_for_index(&index_iterator, index, &opts)) < 0)
		goto done;
...
int git_index_read_tree(
	git_index *index,
	const git_tree *new_tree)
{
	git_iterator *new_iterator = NULL;
	git_iterator_options opts = GIT_ITERATOR_OPTIONS_INIT;
	int error;

	/* TODO: free old tree cache, if any */
	index->tree = NULL;

	if ((error = git_iterator_for_tree(&new_iterator, new_tree, &opts)) < 0 ||
		(error = read_iterator(index, new_iterator, 32)) < 0)
		goto done;

	error = git_tree_cache_read_tree(&index->tree, new_tree, &index->tree_pool);
	...
	const git_index *new_index)
{
	...
	if ((error = git_iterator_for_index(&new_iterator, (git_index *)new_index, &opts)) == 0)
		error = read_iterator(index, new_iterator, new_index->entries.length);
