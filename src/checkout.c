	...
	checkout_data *data, const char *path, unsigned int mode)
{
	...
	/* TODO: we could also test to see if last_mkdir is our direct parent */
	if (data->last_mkdir.size && mkdir_path->size == data->last_mkdir.size &&
		memcmp(mkdir_path->ptr, data->last_mkdir.ptr, data->last_mkdir.size) == 0)
		return 0;

	if ((error = checkout_mkdir(
			data, mkdir_path->ptr, git_repository_workdir(data->repo), mode,
