 ...
 */
int git_process__cmdline(git_buf *out, git_strarray *in)
{
	bool quoted = false;
	char *c;
	size_t i;
	...
	git_process_options *opts)
{
	...
	*out = NULL;
	...
	if ((error = git_process__cmdline(&cmdline, arg)) < 0)
		goto done;

	if (git__utf8_to_16_alloc(&process->appname, arg->strings[0]) < 0 ||
