...
create_profiling_script() {
	...
	if [ "${SYSTEM}" = "Linux" ]; then
		echo -n "cd \"${START_DIR}\" && perf record -F 99 -a -g -o \"${SANDBOX_DIR}/perf.data\" -- \"${CLI_PATH}\" > \"${SANDBOX_DIR}/cli.out\" 2> \"${SANDBOX_DIR}/perf.out\"" >> "${SANDBOX_DIR}/perf.sh"
		for a in "$@"; do
			echo -n " \"${a}\"" >> "${SANDBOX_DIR}/perf.sh"
		done

		echo "" >> "${SANDBOX_DIR}/perf.sh"

		echo "perf report --stdio -n -g folded,0,caller,count -s comm -i \"${SANDBOX_DIR}/perf.data\" | awk '/^ / { comm = \$3 } /^[0-9]/ { print comm \";\" \$2, \$1 }' > \"${SANDBOX_DIR}/perf.data.folded\" 2>> \"${SANDBOX_DIR}/perf.out\"" >> "${SANDBOX_DIR}/perf.sh"
		echo "echo \"${SANDBOX_DIR}/perf.data.folded\"" >> "${SANDBOX_DIR}/perf.sh"
	else
...
exec_profiler() {
	...
	if [ "${SYSTEM}" = "Linux" ]; then
		...
		bash "${PREPARE_SCRIPT}"
		( cd "${START_DIR}" && perf record -F 99 -a -g -o "${SANDBOX_DIR}/perf.data" -- "${TEST_CLI_PATH}" "${GIT_ARGUMENTS[@]}" )
		...
		if [ "${FLAMEGRAPH}" = "" ]; then
			...
			perf report --stdio -n -g folded,0,caller,count -s comm -i "${SANDBOX_DIR}/perf.data" | awk '/^ / { comm = $3 } /^[0-9]/ { print comm ";" $2, $1 }' > "${SANDBOX_DIR}/perf.data.folded"
			perl ./flamegraph.pl "${SANDBOX_DIR}/perf.data.folded" > "${FLAMEGRAPH}"
		fi
	else
		# macos - requires system integrity protection is disabled :(
		# dtrace -s "bash ${TEST_RUN_SCRIPT}" -o filename -n "profile-997 /execname == \"${TEST_CLI}\"/ { @[ustack(100)] = count(); }"
