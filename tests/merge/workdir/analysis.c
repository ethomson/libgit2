...
{
	git_repository *new_repo;
	git_remote *remote;
	git_reference *ref;
	git_merge_head *their_head;
	git_merge_analysis_t analysis;

	cl_git_pass(git_repository_init(&new_repo, "unborn-repo", 0));
	cl_git_pass(git_remote_create(&remote, new_repo, "origin", git_repository_path(repo)));
	cl_git_pass(git_remote_fetch(remote, NULL, NULL));

	cl_git_pass(git_reference_lookup(&ref, new_repo, "refs/remotes/origin/master"));
	cl_git_pass(git_merge_head_from_ref(&their_head, new_repo, ref));

	cl_git_pass(git_merge_analysis(&analysis, new_repo, (const git_merge_head **)&their_head, 1));
