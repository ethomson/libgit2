...
void test_diff_tree__0(void)
{
	...
	cl_git_pass(git_diff_foreach(
		diff, diff_file_cb, diff_hunk_cb, diff_line_cb, diff_binary_cb, &expect));
...
#define EXPECT_STATUS_ADM(ADDS,DELS,MODS) { 0, ADDS, DELS, MODS, 0, 0, 0, 0, 0 }
	...
	for (i = 0; test_expects[i].files > 0; i++) {
		...
		cl_git_pass(git_diff_foreach(
			diff, diff_file_cb, diff_hunk_cb, diff_line_cb, diff_binary_cb, &actual));
...
void test_diff_tree__merge(void)
{
	...
	cl_git_pass(git_diff_foreach(
		diff1, diff_file_cb, diff_hunk_cb, diff_line_cb, diff_binary_cb, &expect));
