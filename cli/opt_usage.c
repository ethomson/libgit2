...
/*
 * This is similar to adopt's function, but modified to understand a
 * terminal's line length and wrap appropriately, using a `git_buf`
 * for storage.
 */
int gitcli_opt_usage_fprint(
	FILE *file,
	const char *command,
	const gitcli_opt_spec specs[])
{
	...
	if ((fd = fileno(file)) >= 0)
		gitcli_console_coords(&console_width, NULL, fd);

	for (spec = specs; spec->type; ++spec) {
		int optional = !(spec->usage & GITCLI_OPT_USAGE_REQUIRED);

		if (spec->usage & GITCLI_OPT_USAGE_HIDDEN)
			continue;
		...
		if (spec->type == GITCLI_OPT_VALUE && spec->alias)
			error = git_buf_printf(&opt, "-%c <%s>", spec->alias, spec->value_name);
		else if (spec->type == GITCLI_OPT_VALUE)
			error = git_buf_printf(&opt, "--%s=<%s>", spec->name, spec->value_name);
		else if (spec->type == GITCLI_OPT_VALUE_OPTIONAL && spec->alias)
			error = git_buf_printf(&opt, "-%c [<%s>]", spec->alias, spec->value_name);
		else if (spec->type == GITCLI_OPT_VALUE_OPTIONAL)
			error = git_buf_printf(&opt, "--%s[=<%s>]", spec->name, spec->value_name);
		else if (spec->type == GITCLI_OPT_ARG)
			error = git_buf_printf(&opt, "<%s>", spec->value_name);
		else if (spec->type == GITCLI_OPT_ARGS)
			error = git_buf_printf(&opt, "<%s...>", spec->value_name);
		else if (spec->type == GITCLI_OPT_LITERAL)
			error = git_buf_printf(&opt, "--");
		else if (spec->alias && !(spec->usage & GITCLI_OPT_USAGE_SHOW_LONG))
		...
		choice = !!((spec+1)->usage & GITCLI_OPT_USAGE_CHOICE);
